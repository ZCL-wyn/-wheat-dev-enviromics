...
# 修复：dplyr版本兼容 + Haplotype转为因子 + 所有select报错
# ============================================================
...
cat(sprintf("Detected %d haplotypes | Top 6: %s\n", 
            nrow(haplotype_counts), paste(top6_haps, collapse = ", ")))
...
# 标记Top6单倍型
haplotype_models$Is_Top6 <- ifelse(haplotype_models$Haplotype %in% top6_haps, "Top6", "Other")
...
p_legend <- ggplot() +
  geom_line(
    ...
    aes(x = x, y = y, color = Haplotype, group = Haplotype)  # 修复：添加group
  ) +
  scale_color_manual(values = top6_cols, name = "Top 6 Haplotypes") +
...
p_main <- ggplot() +
  ...
  # 颜色映射（Top6彩色，其他灰色）
  scale_color_manual(values = hap_colors, name = "Haplotype") +
  labs(
  ...
  theme(
    ...
    legend.position = "none",  # 隐藏主图例，仅显示Top6图例
...
# Save PNG
png(png_out, width = 16, height = 12, units = "in", res = 300)
grid.newpage()
grid.draw(main_grob)
...
# Save PDF
pdf(pdf_out, width = 16, height = 12)
grid.newpage()
grid.draw(main_grob)
pushViewport(viewport(
  x = legend_x,
