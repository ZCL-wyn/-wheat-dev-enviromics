...
# 2. 核心路径配置
future_growthstage_dir <- "C:/Users/Lenovo/Desktop/小麦千粒重文章/001历史气象数据和未来气象数据的整理和met文件的整理和apsim对气象的分割/Future_growthstage"
historical_dir <- "C:/Users/Lenovo/Desktop/小麦千粒重文章/001历史气象数据和未来气象数据的整理和met文件的整理和apsim对气象的分割/03APSIMgrowthstage"

# 历史数据文件路径
historical_file_path <- file.path(historical_dir, "final_weather_data_with_simulation_columns.csv")

# 未来数据文件路径
future_file_path <- file.path(future_growthstage_dir, "future_filtered_weather_data_by_growthstage.csv")
...
required_cols <- c("env", "Wheat.Phenology.Zadok.Stage", "Wheat.Phenology.CurrentStageName", 
                   "Wheat.Phenology.ThermalTime", "YYYYMMDD", "TMEAN", "TMAX", "TMIN")
...
# 7. 重命名Wheat.Phenology.ThermalTime列为GDD_r
message("\n=== 重命名Wheat.Phenology.ThermalTime列为GDD_r ===")

# 重命名为GDD_r，表示APSIM模型输出的物候积温
if ("Wheat.Phenology.ThermalTime" %in% colnames(historical_data)) {
  historical_data <- historical_data %>%
    rename(GDD_r = Wheat.Phenology.ThermalTime)
  message("✅ 历史数据中重命名Wheat.Phenology.ThermalTime为GDD_r")
} else {
  message("⚠️ 历史数据中没有Wheat.Phenology.ThermalTime列")
}

if ("Wheat.Phenology.ThermalTime" %in% colnames(future_data)) {
  future_data <- future_data %>%
    rename(GDD_r = Wheat.Phenology.ThermalTime)
  message("✅ 未来数据中重命名Wheat.Phenology.ThermalTime为GDD_r")
} else {
  message("⚠️ 未来数据中没有Wheat.Phenology.ThermalTime列")
}
...
env_summary <- merged_data %>%
  ...
  summarise(
    ...
    Mean_GDD_r = mean(GDD_r, na.rm = TRUE),
...
phenology_stats <- merged_data %>%
  ...
  summarise(
    ...
    Avg_GDD_r = mean(GDD_r, na.rm = TRUE),
...
message("\n🔧 列重命名：")
message("  - Wheat.Phenology.ThermalTime → GDD_r")
message("  （命名理由：与历史数据文件名一致，表示APSIM模型输出的物候积温）")
