# ==============================================================================
# 完整代码：GW模型方差组分分析（使用TKW表型数据，不包含互作项）
...
output_dir <- "GW_TKW_model_no_interaction"
...
# 1. 基因型方差
cat("1. 计算基因型方差...\n")
if(file.exists("m2_ETA_v_b.bin")) {
  B_v <- readBinMat('m2_ETA_v_b.bin')
  ...
  cat("警告: 未找到基因型效应文件 m2_ETA_v_b.bin\n")
  vG <- NA
  mean_vG <- NA
  se_vG <- NA
  ci_vG <- c(NA, NA)
...
# 2. 环境协变量方差
cat("\n2. 计算环境协变量方差...\n")
if(file.exists("m2_ETA_ec_b.bin")) {
  B_ec <- readBinMat('m2_ETA_ec_b.bin')
  ...
  cat("警告: 未找到环境协变量效应文件 m2_ETA_ec_b.bin\n")
  vEC <- NA
  mean_vEC <- NA
  se_vEC <- NA
  ci_vEC <- c(NA, NA)
...
# 3. 残差方差
cat("\n3. 计算残差方差...\n")
if(file.exists("m2_varE.dat")) {
  varE <- scan("m2_varE.dat")
...
# 3. 验证占比总和
total_prop <- sum(c(prop_genotype, prop_environment, prop_residual), na.rm = TRUE)
...
detailed_results <- data.frame(
  Component = c("Cultivar", "Env.Cov", "Error", "Total_Phenotypic"),
  Mean = c(
...
# 3. 保存模型拟合对象
cat("\n3. 保存模型拟合对象...\n")
save(fm2, file = file.path(output_dir, "GW_model_fit.RData"))
cat("  - 保存GW模型拟合对象: GW_model_fit.RData\n")

# 4. 保存所有方差轨迹数据（如果可用）
cat("\n4. 保存方差轨迹数据...\n")
if(exists("vG") && exists("vEC") && exists("varE") && 
   !any(is.na(vG)) && !any(is.na(vEC)) && !any(is.na(varE))) {
...
report <- paste0(
  ...
  "   - G×W交互作用效应: 无 (此模型不包含互作项)\n",
  "   - 环境协变量处理: W <- scale(W, center=TRUE, scale=TRUE); W <- W/sqrt(ncol(W))\n",
  "   - 删除的表头: Year, Location, YxL, SNPxEC (保留Cultivar, Env. Cov., Error)\n\n",
...
if(exists("vG") && exists("vEC") && exists("varE") && 
  ...
  trace_plot_data <- data.frame(
    Iteration = rep(1:length(vG), 3),
    Variance = c(vG, vEC, varE),
    Component = factor(rep(c("Cultivar", "Env.Cov", "Error"), 
                           each = length(vG)),
                       levels = c("Cultivar", "Env.Cov", "Error"))
  )
  
  p1 <- ggplot(trace_plot_data, aes(x = Iteration, y = Variance, color = Component)) +
    geom_line(alpha = 0.7, linewidth = 0.5) +
    facet_wrap(~ Component, scales = "free_y", ncol = 1) +
    labs(title = "GW模型方差组分轨迹图 (TKW, 无互作)",
         x = "迭代次数",
         y = "方差") +
    theme_minimal() +
  ...
  ggsave(file.path(output_dir, "GW_variance_trace_plots.png"), p1, width = 8, height = 8, dpi = 300)
  cat("  - 保存GW模型方差轨迹图: GW_variance_trace_plots.png\n")
}

# 2. 创建方差组分占比饼图
cat("\n2. 创建方差组分占比饼图...\n")
if(!is.na(prop_genotype) && !is.na(prop_environment) && !is.na(prop_residual)) {
  prop_data <- data.frame(
    Component = c("Cultivar", "Env.Cov", "Error"),
    Proportion = c(prop_genotype, prop_environment, prop_residual),
    Label = paste(c("Cultivar", "Env.Cov", "Error"), 
                  sprintf("%.1f%%", c(prop_genotype, prop_environment, prop_residual)))
  )
...
# 3. 创建置信区间图
cat("\n3. 创建置信区间图...\n")
if(exists("ci_vG") && exists("ci_vEC") && exists("ci_varE") &&
   !any(is.na(ci_vG)) && !any(is.na(ci_vEC)) && !any(is.na(ci_varE))) {
  
  ci_data <- data.frame(
    Component = c("Cultivar", "Env.Cov", "Error"),
    Mean = c(mean_vG, mean_vEC, mean_varE),
    Lower = c(ci_vG[1], ci_vEC[1], ci_varE[1]),
    Upper = c(ci_vG[2], ci_vEC[2], ci_varE[2])
  )
  
  p3 <- ggplot(ci_data, aes(x = Component, y = Mean)) +
    geom_point(size = 3, color = "blue") +
    geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.2, color = "blue") +
    labs(title = "GW模型方差组分估计值及95%置信区间 (TKW, 无互作)",
         x = "方差组分",
         y = "方差") +
    theme_minimal() +
  ...
  ggsave(file.path(output_dir, "GW_variance_CI_plots.png"), p3, width = 8, height = 6, dpi = 300)
...
temp_files <- c("m2_ETA_v_b.bin", "m2_ETA_ec_b.bin", 
                "m2_varE.dat", "m2_ETA_v_varB.dat", "m2_ETA_ec_varB.dat", 
                "m2_mu.dat")
...
cat("\n")
cat(rep("=", 80), "\n")
cat("GW模型方差组分分析完整结束! (无互作项)\n")
cat(rep("=", 80), "\n")
cat("分析时间:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("工作目录:", work_dir, "\n")
cat("输出目录:", output_dir, "\n")
cat("表型数据: TKW (千粒重)\n")
cat("模型类型: GW (基因组+环境协变量，无互作项)\n")
