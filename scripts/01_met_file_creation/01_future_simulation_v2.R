...
#   ✅（新增，按你要求）方案A+方案B：
#        - 方案A：Future_APSIMout/ssp126 与 Future_APSIMout/ssp585 分目录写出
#        - 方案B：每个结果CSV内部追加 scenario/site/sowing_code/growing_season/env_id 字段
...
# =========================
# ✅ 方案A：按 scenario 分输出目录（ssp126 / ssp585）
# =========================
...
# =========================
# ✅ 硬校验：确保每个 site × scenario 在气象数据里都有记录（避免跑到一半才报错）
# =========================
check_pairs <- expand.grid(site = loc_data$site, scenario = default_scenarios, stringsAsFactors = FALSE)

missing_pairs <- check_pairs %>%
  dplyr::rowwise() %>%
  dplyr::mutate(n = sum(corrected_weather$site == site & corrected_weather$scenario == scenario, na.rm = TRUE)) %>%
  dplyr::ungroup() %>%
  dplyr::filter(n == 0)

if (nrow(missing_pairs) > 0) {
  print(missing_pairs)
  stop("❌ 气象数据缺少某些 site×scenario 组合，请先检查 corrected_file 的 site / scenario 列是否完整。")
}
...
cat("未来气候小麦模拟完成总结报告（Linux FULL FINAL FIX v5.1.1：JSON全强写 + CLI智能剔除无效变量 + TT回退 + GDD兜底 + 输出按SSP分目录 + CSV内写scenario标签）\n")
...
cat(" - output_ssp126: ", future_dirs$output_ssp126, "\n", sep = "")
cat(" - output_ssp585: ", future_dirs$output_ssp585, "\n", sep = "")
...
cat("\nSSP 输出区分说明（方案A + 方案B）：\n")
cat(" - 方案A：输出目录分为 Future_APSIMout/ssp126 与 Future_APSIMout/ssp585\n")
cat(" - 方案B：每个结果CSV内部追加 scenario/site/sowing_code/growing_season/env_id 五列\n")
