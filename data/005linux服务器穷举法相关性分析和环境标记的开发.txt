...
# ============================================================
# 10. 可用气象因子 + factor_types（GDD 同时 mean/sum）
# ============================================================
cat("\n=== Step 7: 因子准备 ===\n")

new_possible_factors <- c(
  "GDD_r", "TMEAN", "TMAX", "TMIN", "T2MDEW", "ASSD", "APAR", "DL",
  "PR", "QV2M", "RH", "WS2M", "VPD", "GDD_base0", "GDD_simple",
  "GDD_cardinal", "DTR", "FRUE"
)
...
available_factors <- new_possible_factors[new_possible_factors %in% names(weather_data_clean)]
cat("可用因子:", paste(available_factors, collapse = ", "), "\n")

factor_types <- data.frame(factor = character(), type = character(), stringsAsFactors = FALSE)
for (factor in available_factors) {
  if (grepl("^GDD_", factor)) {
    factor_types <- rbind(factor_types, data.frame(factor = factor, type = "mean"))
    factor_types <- rbind(factor_types, data.frame(factor = factor, type = "sum"))
  } else if (factor %in% c("TMEAN", "TMAX", "TMIN", "T2MDEW", "RH", "WS2M", "VPD",
                           "DTR", "APAR", "DL", "QV2M", "FRUE")) {
    factor_types <- rbind(factor_types, data.frame(factor = factor, type = "mean"))
